    ...
    for epoch in range(opt.epoch):
        ...
        for ii, (img, bbox_, label_, scale) in tqdm(enumerate(dataloader)):
            ...
            if (ii + 1) % opt.plot_every == 0:
                ...
                _bboxes, _labels, _scores = faster_rcnn.predict([ori_img_])
        ...
        faster_rcnn.vis.plot('test_map', eval_result['map'])
        lr_ = faster_rcnn.faster_rcnn.optimizer.param_groups[0]['lr']
        log_info = 'lr:{}, map:{},loss:{}'.format(str(lr_),
                                                  str(eval_result['map']),
                                                  str(faster_rcnn.get_meter_data()))
        faster_rcnn.vis.log(log_info)

        if eval_result['map'] > best_map:
            best_map = eval_result['map']
            best_path = faster_rcnn.save(best_map=best_map)
        if epoch == 9:
            faster_rcnn.load(best_path)
            faster_rcnn.faster_rcnn.scale_lr(opt.lr_decay)
